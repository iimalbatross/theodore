...
// ===================== COUNTDOWN TIMER =====================
const birthdayDate = new Date('2026-05-06T00:00:00'); // CHANGE THIS TO HER BIRTHDAY
const now = new Date();
...
async function startAudioOnLoad() {
    ...
    try {
        // Try immediate autoplay (works on desktop)
        await birthdayAudio.play();
        isPlaying = true;
        vinyl.classList.add('playing');
        playBtn.classList.add('playing');
        ...
        // Show mobile autoplay prompt
        const prompt = document.getElementById('mobile-autoplay-prompt');
        if (prompt) {
            prompt.style.display = 'block';
        }

        const resumeOnFirstGesture = async (event) => {
            try {
                // Ensure audio is loaded before playing
                if (birthdayAudio.readyState < 2) {
                    birthdayAudio.load();
                    await new Promise(resolve => {
                        birthdayAudio.addEventListener('canplay', resolve, { once: true });
                    });
                }
                ...
                window.removeEventListener('scroll', resumeOnFirstGesture);
            } catch (err) {
        ...
        window.addEventListener('touchend', resumeOnFirstGesture, { once: true });
        window.addEventListener('keydown', resumeOnFirstGesture, { once: true });

        // Also try on scroll for mobile
        window.addEventListener('scroll', resumeOnFirstGesture, { once: true });
    }
}
